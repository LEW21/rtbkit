...
BOOST_AUTO_TEST_CASE( test_openrtb_from_appnexus )
{
    ...
    // 96.246.152.18
    // OpenRTB::Device
    BOOST_CHECK_EQUAL(ortbReq->device->ua, "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.5; en-US;rv:1.9.0.3) Gecko/2008092414 Firefox/3.0.3");
    BOOST_CHECK_EQUAL(ortbReq->device->language, "en-US,en;q=0.8");
    ...
    BOOST_CHECK_EQUAL(ortbReq->device->carrier, "101");
    BOOST_CHECK_EQUAL(ortbReq->device->language, "en-US,en;q=0.8");
    ...
    BOOST_CHECK_EQUAL(ortbReq->device->geo->city, "New York");
    BOOST_CHECK_EQUAL(ortbReq->device->geo->zip, "10014");
    BOOST_CHECK_EQUAL(ortbReq->device->geo->ext["dma"].asInt(), 501);
    ...
    BOOST_CHECK_CLOSE(*ortbReq->device->geo->lat, (float)38.7875232696533, 0.0000001);
    BOOST_CHECK_CLOSE(*ortbReq->device->geo->lon, (float)-77.2614831924438, 0.0000001);
    ...
    BOOST_CHECK_EQUAL(*ortbReq->user->yob, 50);
    ...
    BOOST_CHECK_EQUAL(int(*ortbReq->imp.front().banner->pos), 0);
    BOOST_CHECK_EQUAL(*ortbReq->imp.front().bidfloor, 1.0);
